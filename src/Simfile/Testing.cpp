...
void VerifyVector(const Vector<T>& a, const Vector<T>& b, const char* name, const char* subname, Equal eq)
{
	if(a.size() != b.size())
	{
		HudError("%s :: %s mismatch in size: %i and %i", name, subname, a.size(), b.size());
	}
	else
	{
...
static void VerifyVal(double a, double b, const char* name, const char* subname)
{
...
template <typename T>
void VerifyBasic(const T& a, const T& b, const char* name, const char* subname)
{
	if(!(a == b))
	{
...
template <typename T, typename Equal>
void VerifyValue(const T& a, const T& b, const char* name, const char* subname, Equal eq)
{
	if(!eq(a, b))
	{
		HudError("%s :: %s mismatch", name, subname);
...
#define CHECK(memberName)\
	Check(#memberName, a.memberName, b.memberName)
...
{
	return CHECK(effect) && CHECK(file) && CHECK(file2) && CHECK(transition) && CHECK(startBeat) && CHECK(rate);
}
...
static void VerifyTempo(const char* name, const Tempo& a, const Tempo& b)
{
	...
	VERIFY_VECTOR(speeds, Speed){ return CHECK(row) && CHECK(delay) && CHECK(ratio) && CHECK(unit); });
	VERIFY_VECTOR(scrolls, Scroll){ return CHECK(row) && CHECK(ratio) && CHECK(ratio); });
	VERIFY_VECTOR(tickCounts, TickCount){ return CHECK(row) && CHECK(ticks); });
	VERIFY_VECTOR(timeSignatures, TimeSignature){ return CHECK(row) && CHECK(num) && CHECK(denom); });
}
...
static void VerifyShared(const char* name, const Shared& a, const Shared& b)
{
	...
	VERIFY_COMPLEX(displayBpmRange, BpmRange){ return CHECK(min) && CHECK(max); });

	VERIFY_VECTOR(misc, Property){ return CHECK(tag) && CHECK(val); });
}

static void VerifyMetadata(const char* name, const Metadata& a, const Metadata& b)
{
	VERIFY_STR(title);
	VERIFY_STR(titleTr);
	VERIFY_STR(subtitle);
...
static void VerifyChart(const Chart& a, const Chart& b)
{
	...
	{
		return CHECK(row) && CHECK(endrow) && CHECK(col) && CHECK(player) && CHECK(type);
	});
}
...
std::string VerifySaveLoadIdentity(const Simfile& simfile)
{
	...
	else
	{
		...
		{
			HudError("charts mismatch in size: %i and %i", a.charts.size(), b.charts.size());
		}
		else
		{

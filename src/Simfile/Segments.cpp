...
Segment::Segment() : row(0) {}

Segment::Segment(int row) : row(row) {}

// ================================================================================================
// BpmChange.

BpmChange::BpmChange() : bpm(0) {}

BpmChange::BpmChange(int row, double bpm) : Segment(row), bpm(bpm) {}
...
bool IsRedundant(const BpmChange& seg, const BpmChange* prev) {
    // return (prev && prev->bpm == seg.bpm); -- original code before Visual
    // Sync commit
...
template <>
std::string GetDescription(const BpmChange& seg) {
    return Str::val(seg.bpm, 3, 6);
}

static const SegmentMeta BpmChangeMeta = {sizeof(BpmChange),
                                          "BPM change",
                                          "BPM changes",
                                          "Beats per minute",
...
Stop::Stop(int row, double seconds) : Segment(row), seconds(seconds) {}
...
template <>
std::string GetDescription(const Stop& seg) {
    return Str::val(seg.seconds, 3, 6);
}

static const SegmentMeta StopMeta = {sizeof(Stop),
                                     "Stop",
                                     "Stops",
                                     "Duration (seconds)",
...
Delay::Delay(int row, double seconds) : Segment(row), seconds(seconds) {}
...
template <>
std::string GetDescription(const Delay& seg) {
    return Str::val(seg.seconds, 3, 6);
}

static const SegmentMeta DelayMeta = {sizeof(Delay),
                                      "Delay",
                                      "Delays",
                                      "Duration (seconds)",
...
Warp::Warp(int row, int numRows) : Segment(row), numRows(numRows) {}
...
template <>
std::string GetDescription(const Warp& seg) {
    return Str::val(seg.numRows * BEATS_PER_ROW, 3, 6);
}

static const SegmentMeta WarpMeta = {sizeof(Warp),
                                     "Warp",
                                     "Warps",
                                     "Duration (beats)",
...
TimeSignature::TimeSignature() : rowsPerMeasure(192), beatNote(4) {}
...
bool IsRedundant(const TimeSignature& seg, const TimeSignature* prev) {
    return (prev && prev->rowsPerMeasure == seg.rowsPerMeasure &&
            prev->beatNote == seg.beatNote);
}

template <>
bool IsEquivalent(const TimeSignature& seg, const TimeSignature& other) {
    return (other.rowsPerMeasure == seg.rowsPerMeasure &&
            other.beatNote == seg.beatNote);
}
...
TickCount::TickCount() : ticks(0) {}

TickCount::TickCount(int row, int ticks) : Segment(row), ticks(ticks) {}
...
template <>
std::string GetDescription(const TickCount& seg) {
...
static const SegmentMeta TickCountMeta = {sizeof(TickCount),
                                          "Tick count",
                                          "Tick counts",
                                          "Hold ticks per beat",
...
Combo::Combo() : hitCombo(0), missCombo(0) {}
...
bool IsRedundant(const Combo& seg, const Combo* prev) {
    return (prev && prev->hitCombo == seg.hitCombo &&
            prev->missCombo == seg.missCombo);
}
...
static const SegmentMeta ComboMeta = {sizeof(Combo),
                                      "Combo segment",
                                      "Combo segments",
                                      "Hit multiplier\nMiss multiplier",
...
Speed::Speed() : ratio(1), delay(0), unit(0) {}
...
bool IsRedundant(const Speed& seg, const Speed* prev) {
    return (prev && prev->ratio == seg.ratio && prev->delay == seg.delay &&
            prev->unit == seg.unit);
}

template <>
bool IsEquivalent(const Speed& seg, const Speed& other) {
    return (other.ratio == seg.ratio && other.delay == seg.delay &&
            other.unit == seg.unit);
}
...
Scroll::Scroll() : ratio(1) {}

Scroll::Scroll(int row, double ratio) : Segment(row), ratio(ratio) {}
...
template <>
std::string GetDescription(const Scroll& seg) {
...
static const SegmentMeta ScrollMeta = {sizeof(Scroll),
                                       "Scroll segment",
                                       "Scroll segments",
...
Fake::Fake(int row, int numRows) : Segment(row), numRows(numRows) {}
...
template <>
std::string GetDescription(const Fake& seg) {
    return Str::val(seg.numRows * BEATS_PER_ROW, 3, 6);
}
...
static const SegmentMeta FakeMeta = {sizeof(Fake),
                                     "Fake segment",
                                     "Fake segments",
                                     "Duration (beats)",
                                     RGBAtoColor32(128, 128, 51, 255),
...
template <>
std::string GetDescription(const Label& seg) {
    return seg.str;
}
...
static const SegmentMeta LabelMeta = {sizeof(Label),
                                      "Label",
                                      "Labels",
                                      "Description",
...
const SegmentMeta* Segment::meta[NUM_TYPES] = {
    &BpmChangeMeta,     &StopMeta,      &DelayMeta, &WarpMeta,
    &TimeSignatureMeta, &TickCountMeta, &ComboMeta, &SpeedMeta,
    &ScrollMeta,        &FakeMeta,      &LabelMeta,
};

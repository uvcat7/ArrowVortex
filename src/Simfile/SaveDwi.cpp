...
static void EmitNoteChars(std::ofstream& file, const DwiNote* notes, int count)
{
	if(count == 1)
...
static void EmitNoteSlot(std::ofstream& file,
	...
	DwiNote* holdHeads, int nHoldHeads)
{
	...
	else if(nTaps == 1 && nHoldHeads == 1)
	{
		...
		writef(file, "%c!%c", DwiPairToChar(taps[0], holdHeads[0]), DwiNoteToChar(holdHeads[0]));
	}
	else
	{
...
static void WriteNoteDataForPad(std::ofstream& file, const NoteList& notes, int numCols, int pad)
{
	...
	for(int measure = 0; measure <= lastMeasure; measure++)
	{
		...
		if(mode == 1)      writef(file, "(");
		else if(mode == 2) writef(file, "[");
		else if(mode == 3) writef(file, "{");
		else if(mode == 4) writef(file, "`");
...
bool SaveDwi(const Simfile* sim, bool backup)
{
	...
	if(backup && fs::exists(path))
	{
		...
		if(error)
			HudError("Could not backup \"%s\".", pathToUtf8(path.filename()).c_str());
	}
	...
	// BPM changes after row 0 go into CHANGEBPM as a comma-separated beat=bpm list.
	{
		...
		for(auto it = bpmBegin; it != bpmEnd; ++it)
		{
			...
			if(first) { writef(file, "#CHANGEBPM:"); first = false; }
			else writef(file, ",");
	...
	// Stops go into FREEZE as a comma-separated beat=milliseconds list.
	{
		...
		{
			if(first) { writef(file, "#FREEZE:"); first = false; }
			else writef(file, ",");
			writef(file, "%.6f=%.6f", RowToDwiBeat(it->row), it->seconds * 1000.0);
		}
		if(!first) writef(file, ";\n");
	}
	...
	// sim->charts is already sorted beginner to expert by SimfileMan::sortCharts().

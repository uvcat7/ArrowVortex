...
static void ConvertNotes(Simfile* sim, OsuFile& osu, Chart& chart) {
    ...
    if (!std::is_sorted(osu.hitObjects.begin(), osu.hitObjects.end(),
                        LessThan)) {
...
std::string& err)
{
        ...
        for(uint32_t i = 0; i < global_info.number_entry; ++i)
        {
                ...
                if(unzGetCurrentFileInfo(zip, &file_info, buffer, 512, 0, 0, 0,
0) == UNZ_OK)
                {
                        ...
                        if(filename.hasExt("osu"))
                        {
                                while(true)
                                {
                                        int read = unzReadCurrentFile(zip,
buffer, 512); if(read <= 0) break; Str::append(str, buffer, read);
                                }
                                if(str.len())
                                {
...
bool LoadOsu(const std::string& path, Simfile* sim) {
    ...
    for (auto file : files) {
        if (file->gameMode == OSU && !firstOsuFile) firstOsuFile = file;

        if (file->gameMode == OSUMANIA && !firstOsuManiaFile)
            firstOsuManiaFile = file;

        if ((file->gameMode == OSU || file->gameMode == OSUMANIA) &&
            file->filename == sim->file)
            mainFile = file;
    }
    ...
    for (auto file : files) {
        if (file->gameMode == OSUMANIA && file->hitObjects.size()) {
            ...
            chart->style =
                gStyle->findStyle(file->chartVersion, file->numCols, 1);
    ...
    // After all charts are loaded, check which set of difficulties best matches
    // the loaded set.

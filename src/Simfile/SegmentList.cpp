...
static void VerifyAdd(Segment::Type type, const Segment* add, const Segment*& prev, int& prevRow)
{
	...
	if(add->row <= prevRow)
	{
		if(add->row == prevRow)
		{
			HudWarning("Bug: added segment is on the same row as the previous segment.");
		}
		else
		{
			HudWarning("Bug: added segment is on a row before the previous segment.");
		}
	}
...
void SegmentList::prepareEdit(const List& inAdd, const List& inRem,
	List& outAdd, List& outRem, int regionBegin, int regionEnd)
{
	...
	for(; it != itEnd; ++it)
	{
		...
		// Add all segments on the to-be-added list that occur before this segment.
		...
		// Check if the next segment on the to-be-added list is on the same row as this segment.
		if(nextAddRow == row)
		{
			// If the added segment is equivalent to this segment, prevent removal of the segment.
			if(meta->isEquivalent(it.ptr, add.ptr))
			{
				keep = true;
			}
			else // The added segment is different and overwrites this segment.
			{
		...
		// Check if this segment has become redundant due to previous additions/removals.

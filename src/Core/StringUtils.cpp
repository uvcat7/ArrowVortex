...
void Str::append(std::string& s, char c) { s.append(1, c); }

void Str::append(std::string& s, const std::string& str) { s.append(str); }

void Str::append(std::string& s, const char* str) { s.append(str); }
...
double Str::readTime(const std::string& s, double alt) {
    ...
    switch (time.size()) {
        case 3:
            v = (readInt(time[0]) * 3600) + (readInt(time[1]) * 60) +
                readDouble(time[2]);
            break;
        case 2:
...
size_t Str::nextChar(const std::string& s, size_t pos) {
    ...
    do {
        ++pos;
    } while (pos < len && (s.at(pos) & 0xC0) == 0x80);
    return pos;
}
...
size_t Str::prevChar(const std::string& s, size_t pos) {
    ...
    do {
        --pos;
    } while (pos >= 0 && (s.at(pos) & 0xC0) == 0x80);
    return pos;
}
...
int Str::compare(const char* a, const char* b) { return strcmp(a, b); }
...
int Str::icompare(const char* a, const char* b) { return fastnocasecmp(a, b); }
...
bool Str::equal(const char* a, const char* b) { return compare(a, b) == 0; }
...
bool Str::iequal(const char* a, const char* b) { return icompare(a, b) == 0; }
...
static const uint8_t* ParseMultiplicationOperand(const uint8_t* p,
                                                 double& out) {
    if (*p == '(') {
        p = ParseNestedExpression(SkipWs(++p), out);
        if (*p == ')') p = SkipWs(++p);
...
std::vector<std::string> Str::split(const std::string& s, const char* lim,
                                    bool trim, bool skip) {
    std::vector<std::string> out;

...
TT inline void ToggleFlags(T& bits, int flags)
{
	bits ^= flags;
}

TT inline void SetFlags(T& bits, int flags)
{
	bits |= flags;
}

TT inline void UnsetFlags(T& bits, int flags)
{
	bits &= ~flags;
}
...
TT inline const T& min(const T& a, const T& b)
{
	return (a < b) ? a : b;
}

TT inline const T& max(const T& a, const T& b)
{
	return (b < a) ? a : b;
}
...
TT inline T CenterX(const rectt<T>& r)
{
	return r.x + r.w / 2;
}

TT inline T CenterY(const rectt<T>& r)
{
	return r.y + r.h / 2;
}

TT inline T RightX(const rectt<T>& r)
{
	return r.x + r.w;
}

TT inline T BottomY(const rectt<T>& r)
{
	return r.y + r.h;
}

TT inline vec2t<T> Pos(const rectt<T>& r)
{
	return {r.x, r.y};
}

TT inline vec2t<T> Size(const rectt<T>& r)
{
	return {r.w, r.h};
}

TT inline rectt<T> SideT(const rectt<T>& r, T h)
{
	return {r.x, r.y, r.w, h};
}

TT inline rectt<T> SideL(const rectt<T>& r, T w)
{
...
TT inline void SetSize(rectt<T>& r, const vec2t<T>& s)
{
	r.w = s.x, r.h = s.y;
}

TT inline void SetPos(rectt<T>& r, const vec2t<T>& p)
{
	r.x = p.x, r.y = p.y;
}
...
TT inline vec2t<T> ToVec2(T x, T y)
{
	return {x, y};
}
...
inline colorf ToColor(float l, float a = 1.0f)
{
	return {l, l, l, a};
}

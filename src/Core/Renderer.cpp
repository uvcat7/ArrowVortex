...
static void loadShaders()
{
	...
	const char* colorHsvShaderFrag =
		...
		"gl_FragColor = vec4(c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y), c.a); }";

	const char* textureShaderVert =
		"varying vec4 c; varying vec2 uvs;"
		"void main() { c = gl_Color; uvs = gl_MultiTexCoord0.xy; gl_Position = gl_ModelViewProjectionMatrix * gl_Vertex; }";
	...
	const char* alphaTexShaderFrag =
		"uniform sampler2D tex; varying vec4 c; varying vec2 uvs;"
		"void main() { gl_FragColor = vec4(c.rgb, c.a * texture2D(tex, uvs).r); }";

	if(Shader::isSupported())
	{
		Shader& col = RI->shaders[Renderer::SH_COLOR];
		col.load(colorShaderVert, colorShaderFrag, nullptr, "Renderer::colorShader");
		
		Shader& hsv = RI->shaders[Renderer::SH_COLOR_HSV];
		hsv.load(colorShaderVert, colorHsvShaderFrag, nullptr, "Renderer::hsvShader");

		Shader& tex = RI->shaders[Renderer::SH_TEXTURE];
		tex.load(textureShaderVert, textureShaderFrag, nullptr, "Renderer::textureShader");

		Shader& atex = RI->shaders[Renderer::SH_TEXTURE_ALPHA];
		atex.load(textureShaderVert, alphaTexShaderFrag, nullptr, "Renderer::alphaTexShader");
...
void Renderer::drawQuads(int numQuads, const int* pos, const color32* col)
{
	...
	while(numQuads > BATCH_QUAD_LIMIT) numQuads -= FlushQuads(GL_INT, pos, nullptr, col);
	glDrawElements(GL_TRIANGLES, numQuads * 6, GL_UNSIGNED_INT, RI->quadIndices);
}
...
void Renderer::drawQuads(int numQuads, const int* pos, const float* uvs)
{
	...
	while(numQuads > BATCH_QUAD_LIMIT) numQuads -= FlushQuads(GL_INT, pos, uvs);
	glDrawElements(GL_TRIANGLES, numQuads * 6, GL_UNSIGNED_INT, RI->quadIndices);
}
...
void Renderer::drawQuads(int numQuads, const int* pos, const float* uvs, const color32* col)
{
	...
	while(numQuads > BATCH_QUAD_LIMIT) numQuads -= FlushQuads(GL_INT, pos, uvs, col);
	glDrawElements(GL_TRIANGLES, numQuads * 6, GL_UNSIGNED_INT, RI->quadIndices);
}
...
void Renderer::drawQuads(int numQuads, const float* pos, const float* uvs, const color32* col)
{
	...
	while(numQuads > BATCH_QUAD_LIMIT) numQuads -= FlushQuads(GL_FLOAT, pos, uvs, col);
	glDrawElements(GL_TRIANGLES, numQuads * 6, GL_UNSIGNED_INT, RI->quadIndices);
}
...
{
	Renderer::drawQuads(BATCH_QUAD_LIMIT - RI->quadsLeft,
		(int*)RI->batchPos, (float*)RI->batchUvs);
	RI->quadsLeft = BATCH_QUAD_LIMIT;
}
...
void QuadBatchT::flush()
{
	FlushIT();
}

void QuadBatchTC::flush()
{
	FlushITC();
}

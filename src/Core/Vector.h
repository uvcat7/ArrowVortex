...
class Vector {
   public:
    ...
    /// Constructs an empty vector. Reserves memory for the given number of
    /// elements.
    Vector(int capacity);

    /// Constructs a vector of the given size, with all elements copy
    /// constructed from v.
    ...
    /// Reduces the capacity to the minimum size required to store all current
    /// elements.
    void squeeze();
    ...
    /// Returns true if the vector contains one or more elements that equal
    /// value.
    bool contains(const T& value) const;

    /// Returns the index of the first element on or after pos that equals
    /// value. If no matching element is found, the vector size is returned.
    ...
    /// Returns the final element of the vector; does not perform an
    /// out-of-bounds check. To avoid failure, make sure size is non-zero before
    /// using this function.
    ...
    /// Returns a reference to the value at index i; does not perform an
    /// out-of-bounds check.
    ...
    /// Returns a const reference to the value at index i; does not perform an
    /// out-of-bounds check.
...
template <typename T>
Vector<T>::Vector(int n) : data_(nullptr), size_(0), capacity_(0) {
    reserve(n);
}
...
template <typename T>
Vector<T>::Vector(const Vector& v) : data_(nullptr), size_(0), capacity_(0) {
    assign(v);
}
...
template <typename T>
Vector<T>::Vector(int n, const T& v) : data_(nullptr), size_(n), capacity_(0) {

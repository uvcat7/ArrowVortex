...
#define ForXmrAttribs(it, node) \
	for(XmrAttrib* it = (node) ? (node)->attrib() : nullptr; it; it = it->next())
...
#define ForXmrAttribsNamed(it, node, name) \
	for(XmrAttrib* it = (node) ? (node)->attrib(name) : nullptr; it; it = it->next(name))
...
#define ForXmrNodesNamed(it, node, name) \
	for(XmrNode* it = (node) ? (node)->child(name) : nullptr; it; it = it->next(name))

/// Enumeration of results that can be returned by load/save functions.
enum XmrResult
{
	XMR_SUCCESS = 0,             ///< The XMR was loaded/saved succesfully.
	XMR_COULD_NOT_OPEN_FILE,     ///< The file could not be opened for reading/writing.
	XMR_EXPECTED_NODE_OR_ATTRIB, ///< A '{' or '=' was expected, but did not occur.
...
enum XmrQuoteSetting
{
	...
	XMR_QUOTE_WHEN_TEXT,      ///< Strings that are not formatted as numbers are quoted.
	XMR_QUOTE_WHEN_NECESSARY, ///< Strings that have invalid name characters are quoted.
};

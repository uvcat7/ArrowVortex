...
namespace Text
{
	...
	vec2i arrange(Align align, const TextStyle& style, int maxLineWidth, const char* text);
	...
	/// Creates a glyph image that can be embedded in text by using the {g:name} markup.
	bool createGlyph(const char* name, const Texture& texture, int dx = 0, int dy = 0, int advance = 0);
};

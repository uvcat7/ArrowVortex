...
#define PROC(name) name = (decltype(name))wglGetProcAddress(#name); if(!name) { missing.push_back(#name); ++numMissing; }
#define PROC_OPT(name) name = (decltype(name))wglGetProcAddress(#name); if(!name) { missing.push_back(#name); }

//#define PROC(name) name = nullptr; if(!name) { missing.push_back(#name); ++numMissing; }
//#define PROC_OPT(name) name = nullptr; if(!name) { missing.push_back(#name); }
...
EXT(glShaderSource, void)(GLuint shader, GLsizei count, const char* const* str, const GLint* len);
EXT(glGetShaderiv, void)(GLuint shader, GLenum pname, GLint* param);
EXT(glGetShaderInfoLog, void)(GLuint shader, GLsizei bufSize, GLsizei* len, char* str);
...
EXT(glUniform4f, void)(GLint loc, GLfloat v0, GLfloat v1, GLfloat v2, GLfloat v3);
...
EXT(glGetProgramInfoLog, void)(GLuint program, GLsizei bufSize, GLsizei* len, char* str);
...
void Shader::uniform1f(int loc, float val)
{
	glUniform1f(loc, val);
}

void Shader::uniform2f(int loc, float x, float y)
{
	glUniform2f(loc, x, y);
}

...
void Canvas::Data::draw(float* buf, int w, int h, const areaf& area,
                        DistanceFunc* func) {
...
void Canvas::setOuterGlow(float size) { data_->outerGlow = size; }

void Canvas::setInnerGlow(float size) { data_->innerGlow = size; }

void Canvas::setFill(bool enabled) { data_->outline = !enabled; }

void Canvas::setBlendMode(BlendMode bm) { data_->blendMode = bm; }
...
void Canvas::setColor(const colorf& c) { setColor(c, c, c, c); }

void Canvas::setColor(const colorf& t, const colorf& b) {
    setColor(t, t, b, b);
}

void Canvas::setColor(const colorf& tl, const colorf& tr, const colorf& bl,
                      const colorf& br) {
...
void Canvas::clear(float l) {
    for (float *p = canvas_data_, *end = p + canvas_width_ * canvas_height_ * 4;
         p < end;) {
        *p = l;
        ++p;
        *p = l;

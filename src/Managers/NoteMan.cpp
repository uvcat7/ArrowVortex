...
struct NotesManImpl : public NotesMan {
    ...
    void myUpdateFakedNotes() {
        ...
        for (; it != end; ++it) {
            for (; note != noteEnd; ++note) {
                note->isFake =
                    note->type == NOTE_FAKE ||
                    note->row >= it->row && note->row <= it->row + it->length;
            }
        }
        for (; note != noteEnd; ++note) {
    ...
    void myApplyNotes(Chart* chart, const NoteList& add, const NoteList& rem,
                      bool firstTime) {
    ...
    void myQueueAddNote(Note note) {
        ...
        gHistory->addEntry(myApplyAddNoteId, stream.data(), stream.size(),
                           myChart);
    }

    static std::string ApplyAddNote(ReadStream& in, History::Bindings bound,
                                    bool undo, bool redo) {
        std::string msg;
        Note note;
        DecodeNote(in, note);
    ...
    void myQueueRemoveNote(Note note) {
        WriteStream stream;
        EncodeNote(stream, note);
        gHistory->addEntry(myApplyRemNoteId, stream.data(), stream.size(),
                           myChart);
    }

    static std::string ApplyRemoveNote(ReadStream& in, History::Bindings bound,
                                       bool undo, bool redo) {
        std::string msg;
        Note note;
        DecodeNote(in, note);
    ...
    void myQueueChangeNotes(const NoteEditResult& edit,
                            const EditDescription* desc) {
        WriteStream stream;
        edit.add.encode(stream, false);
        edit.rem.encode(stream, false);
        stream.write(desc);
        gHistory->addEntry(myApplyChangeNotesId, stream.data(), stream.size(),
                           myChart);
    }

    static std::string ApplyChangeNotes(ReadStream& in, History::Bindings bound,
                                        bool undo, bool redo) {
        std::string msg;
    ...
    void myItemizeInsertRows(WriteStream& stream, Chart* chart, int startRow,
                             int numRows) {
        NoteEdit edit;
    ...
    void pasteFromClipboard(bool insert) {
        ...
        NoteEdit edit;
        if (timeBased) {
            edit.add.decode(stream, gTempo->getTimingData(),
                            gView->getCursorTime());
        } else {
...
void NotesMan::create() { gNotes = new NotesManImpl; }

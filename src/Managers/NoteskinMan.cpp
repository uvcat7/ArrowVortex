...
static void ReadRange(bool* flags, const XmrNode* n, const char* attrib,
                      int num, bool areRows) {
...
static void ParseSpriteAttrib(const XmrNode* node, const char* attrib, int* dst,
                              int numCols) {
    int v;
...
static void LoadNoteskin(NoteskinImpl* skin, const SkinType& type) {
    ...
    skin->rightX =
        skin->colX[numCols - 1] + skin->recepOff[numCols - 1].width / 2;
}
...
struct NoteskinManImpl : public NoteskinMan {
        ...
        for (auto skin : File::findDirs("noteskins", false)) {
            ...
            for (auto files : File::findFiles(skin, false, "txt")) {
                ...
                if (id == "all-styles") {
                    ...
                    if (!style) {
                        HudError(
                            "Could not find style '%s' used in noteskin: %s",
    ...
    void update(Chart* chart) override {
        ...
        if (style) {
            ...
            // Find the noteskin type with the highest priority that supports
            // the style.

...
/// A set of multiple threads that perform the same task, which split into
/// items.
class ParallelThreads {
   public:
    ...
    /// Creates several threads, which concurrently start calling "exec". Once
    /// "exec" has been called for every value starting from zero up to numItems

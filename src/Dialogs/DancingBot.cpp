...
struct FeetPlanner {
    ...
    void setFeet(const ExpandedNote* l, const ExpandedNote* r, vec2i posL,
                 vec2i posR);
...
void FeetPlanner::setFeet(const ExpandedNote* l, const ExpandedNote* r,
                          vec2i posL, vec2i posR) {
    int il = l - noteBegin;
    int ir = r - noteBegin;
...
void FeetPlanner::plan(int pn) {
    ...
    while (n != end || nextNote[0].a) {
        ...
        // Skip notes that were already planned (as part of a
        // crossover/footswitch).
        if (skipNotes > 0) {
            --skipNotes;
...
void DialogDancingBot::myCreateWidgets() {
    ...
    footswitch->setTooltip(
        "If enabled, the dancing bot will attempt footswitches");
    ...
    crossover->setTooltip(
        "If enabled, the dancing bot will attempt crossovers");
}
...
void DialogDancingBot::onDoCrossoversChanged() { myAssignFeetToNotes(); }

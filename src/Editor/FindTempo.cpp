...
struct GapData {
    GapData(int numThreads, int maxInterval, int downsample, int numOnsets,
            const Onset* onsets);
    ~GapData();
...
GapData::GapData(int numThreads, int bufferSize, int downsample, int numOnsets,
                 const Onset* onsets)
    : numOnsets(numOnsets),
      onsets(onsets),
...
// given gap position.
static real GapConfidence(const GapData& gapdata, int threadId, int gapPos,
                          int interval) {
    int numOnsets = gapdata.numOnsets;
    int windowSize = gapdata.windowSize;
    int halfWindowSize = windowSize / 2;
    const real* window = gapdata.window;
    const real* wrappedOnsets =
        gapdata.wrappedOnsets + gapdata.bufferSize * threadId;
    real area = 0.0;
...
static real GetConfidenceForInterval(const GapData& gapdata, int threadId,
                                     int interval) {
    ...
    for (int i = 0; i < numOnsets; ++i) {
        ...
        real confidence =
            GapConfidence(gapdata, threadId, pos, reducedInterval);
        int offbeatPos = (pos + reducedInterval / 2) % reducedInterval;
        confidence +=
            GapConfidence(gapdata, threadId, offbeatPos, reducedInterval) * 0.5;
...
static real GetConfidenceForBPM(const GapData& gapdata, int threadId,
                                IntervalTester& test, real bpm) {
    int numOnsets = gapdata.numOnsets;
    const Onset* onsets = gapdata.onsets;
    ...
    for (int i = 0; i < numOnsets; ++i) {
        ...
        confidence +=
            GapConfidence(gapdata, threadId, offbeatPos, interval) * 0.5;
    ...
    if (numThreads > 1) {
        struct IntervalThreads : public ParallelThreads {
            ...
            void exec(int i, int thread) {
                ...
                test->fitness[index] = std::max(
                    0.001,
                    GetConfidenceForInterval(*gapdata, thread, interval));
            }
        };
        ...
        for (int i = 0; i < numCoarseIntervals; ++i) {
            ...
            int interval = test.minInterval + index;
            test.fitness[index] =
                std::max(0.001, GetConfidenceForInterval(gapdata, 0, interval));
        }
    }
}
...
static void CalculateBPM(SerializedTempo* data, Onset* onsets, int numOnsets) {
    ...
    GapData* gapdata = new GapData(data->numThreads, test.maxInterval,
                                   IntervalDownsample, numOnsets, onsets);
    ...
    delete gapdata;
    gapdata =
        new GapData(data->numThreads, test.maxInterval, 0, numOnsets, onsets);

...
struct Music {
    ...
    /// Cuts the given number of seconds off the front and the end of the
    /// song, pads either end with silence, optionally eases the song in and
    /// out over fades, and moves the simfile timing along with it.
    /// The result is written to a file of its own and played in place of
    /// the song, but nothing is committed until it is applied.
    virtual void startAudioTrim(double cutSeconds, double cutEndSeconds,
                                double fadeInSeconds, double fadeOutSeconds,
                                double padStart, double padEnd,
                                bool keepOriginal, bool saveSimfile) = 0;

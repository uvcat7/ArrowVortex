...
struct MusicImpl : public Music, public MixSource {
    ...
    void startAudioTrim(double cut, double cutEnd, double fadeIn,
                        double fadeOut, double padStart, double padEnd,
                        bool keepOriginal, bool saveSimfile) override {
        if (gSimfile->isClosed()) {
            return;
        } else if (!mySamples.isCompleted()) {
            HudNote("Wait for the music to finish loading.");
            ...
            HudNote("Conversion is currently in progress.");
            return;
        } else if (cut <= 0.0 && cutEnd <= 0.0 && fadeIn <= 0.0 &&
                   fadeOut <= 0.0 && padStart <= 0.0 && padEnd <= 0.0) {
        ...
        myAudioConversionThread->trimEnd = cutEnd;

...
static int open_input_file(const char* filename,
    ...
    if ((error = avformat_find_stream_info(*input_format_context, nullptr)) <
        0) {
        av_make_error_string(errbuf, AV_ERROR_MAX_STRING_SIZE, error);
...
static int open_output_file(const char* filename,
    ...
    if (!((*output_format_context)->oformat =
              av_guess_format(nullptr, filename, nullptr))) {
...
static int init_resampler(AVCodecContext* input_codec_context,
    ...
    error = swr_alloc_set_opts2(
        ...
        input_codec_context->sample_rate, 0, nullptr);
    if (error < 0) {
...
/* Where the music stops when the end is cut off, counted the same way. A
 * zero means the song runs to wherever the input does. */
static int64_t samples_to_write = 0;
...
static int load_encode_and_write(AVAudioFifo* fifo,
    ...
    /* The last frame of a song whose end is cut off is only as long as
     * what is left of it. */
    if (samples_to_write > 0 &&
        samples_written + frame_size > samples_to_write) {
        frame_size = static_cast<int>(samples_to_write - samples_written);
    }
...
void OggConversionThread::exec() {
    ...
    if (init_fifo(&fifo, output_codec_context)) {
        error = "Failed to initialize the FIFO buffer.";
        cleanup(fifo, input_format_context, output_format_context,
                input_codec_context, output_codec_context, resample_context);
        return;
    }
    ...
    if (write_output_file_header(output_format_context)) {
        error = "Failed to write the output file header.";
        cleanup(fifo, input_format_context, output_format_context,
                input_codec_context, output_codec_context, resample_context);
        return;
    }
    ...
    if (samples > 0 && input_codec_context->sample_rate > 0) {
        int64_t music = static_cast<int64_t>(samples) *
                            output_codec_context->sample_rate /
                            input_codec_context->sample_rate -
                        samples_to_drop;
        if (trimEnd > 0.0) {
            music -= llround(trimEnd * output_codec_context->sample_rate);
        }
        if (music > 0) music_end = music_start + music;
    }
    if (music_end <= 0) fade_out_samples = 0;

    /* Cutting the end is a limit on what gets written, and the fade-out
     * lands on the new end rather than on the one the input had. */
    if (trimEnd > 0.0 && music_end > 0) samples_to_write = music_end;
    ...
    if (output_codec_context->frame_size == 0) {
        ...
        frames = samples / (output_codec_context->frame_size);
    }
    ...
    while (true) {
        ...
        while (av_audio_fifo_size(fifo) >= output_frame_size ||
            ...
            i_frame++;
            progress = static_cast<int>((i_frame / static_cast<float>(frames)) *
                                        100.0f);
            if (load_encode_and_write(fifo, output_format_context,
                                      output_codec_context)) {
                error =
                    "Failed to encode a frame from the FIFO to the output "
                    "file.";
                cleanup(fifo, input_format_context, output_format_context,
                        input_codec_context, output_codec_context,
                        resample_context);
        ...
        if (samples_to_write > 0 && samples_written >= samples_to_write) {
            finished = 1;
        }
        if (finished) {
            int data_written;
    ...
    if (write_output_file_trailer(output_format_context)) {
        error = "Failed to write output file trailer.";
        cleanup(fifo, input_format_context, output_format_context,
                input_codec_context, output_codec_context, resample_context);
        return;
    }
